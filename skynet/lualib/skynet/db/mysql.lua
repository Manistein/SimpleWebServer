...
local MAX_PACKET_LENGTH = 2^24-1
...
local function _compose_packet(self, req)
    local len = #req
    if len >= MAX_PACKET_LENGTH then
        local pkts = {}

        local pktn = mathfloor(len/MAX_PACKET_LENGTH) + 1
        for i=1,pktn do
            self.packet_no = self.packet_no + 1

            local si, ei = (i-1)*MAX_PACKET_LENGTH +1, math.min(len, i*MAX_PACKET_LENGTH)
            local subreq = string.sub(req, si, ei)
            local size = #subreq
            table.insert(pkts, strpack("<I3Bc" .. size, size, self.packet_no, subreq))
        end
        return table.concat(pkts)
    else
        self.packet_no = self.packet_no + 1
        local size = #req
...
local function read_result(self, sock)
    ...
    while true do
        ...
        -- 拼接big packet
        if #packet == MAX_PACKET_LENGTH then
            bigrow = bigrow or {}
            table.insert(bigrow, packet)
        else
            if bigrow and #bigrow>0 then
                table.insert(bigrow, packet)
                packet = table.concat(bigrow)
                bigrow = nil
            end
