...
function COMMAND.e(address, cmd, ...)
	local addrs = {}
	local prefix = address:sub(1,1)
	if prefix == '.' then
		local addr = assert(skynet.localname(address), "Not a valid name")
	...
	elseif prefix == ':' then
		...
		if addr then
			addr = addr | (skynet.harbor(skynet.self()) << 24)
			table.insert(addrs, addr)
		else
			local addr_list = skynet.call(".launcher", "lua", "LIST")
			for k,v in pairs(addr_list) do
				--skynet.error(string.format("address=%s, info=%s", k, v))
				local split = split_cmdline(v)
				if split[2] == address then
	...
	assert(#addrs>0, "No service address")
	for _,v in ipairs(addrs) do
		skynet.error("address = ", v)
		local code_str = table.concat({...}, " ")
		local ok, output = skynet.call(v, "debug", "EXEC", cmd, code_str)
		if ok == false then
			error(output)
		end
	end
